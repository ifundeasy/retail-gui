// Created by: Gary Skiba (Austin, TX);
// Modified by: Afa Rappresent
// From: https://www.sencha.com/forum/showthread.php?140701-ExtJs4-Checkbox-List-Combo
    ...
    listConfig: {
        ...
        listeners: {
            select: function (me, rec) {
                Ext.getCmp(me.view.checkboxComboId).last = rec;
            },
                        ...
                        switch (allState) {
                            case 0:		// None
                                if (combo.allSelected) {
                                    ...
                                    setTimeout(function () { // suspendEvent is ignored, so we hack using a timer
                                        me.deselectAll(true);
                                        combo.setValue([]);
                                    }, 1);
                                } else {
                            ...
                            case 2:		// Some (gray out the ALL item)
                                var isAll = combo.last.index === 0 || combo.last.internalId == 'ext-record-1';
                                if (isAll) {
                                    combo.allSelected = true;
                                    allItem.removeCls(grayCls);
                                    setTimeout(function () { me.selectAll(false); }, 1); // suspendEvent is ignored, so we hack using a timer
