Ext.define('A.controller.transc.Purchase', {
    ...
    views: ['transc.Purchase', 'transc.PurchaseUnitWindow'],
    refs: [
        {ref: 'purchaseUnitWindow', selector: 'transcpurchase purchaseUnitWindow'},
        //
        {ref: 'masterGrid', selector: 'transcpurchase grid[prop="purchasemaster"]'},
        {ref: 'detailGrid', selector: 'transcpurchase grid[prop="purchasedetail"]'},
        {ref: 'totalItemsLabel', selector: 'transcpurchase grid[prop="purchasedetail"] toolbar label[todo="totalitems"]'},
        {ref: 'totalMoneyLabel', selector: 'transcpurchase grid[prop="purchasedetail"] toolbar label[todo="totalmoney"]'},
        {ref: 'productField', selector: 'transcpurchase grid[prop="purchasedetail"] toolbar textfield[todo="valueFilter"]'},
        {ref: 'addProductBtn', selector: 'transcpurchase grid[prop="purchasedetail"] toolbar button[todo="add"]'},
        {ref: 'deleteProductsBtn', selector: 'transcpurchase grid[prop="purchasedetail"] toolbar button[todo="delete"]'}
    ],
    events: {
        'transcpurchase textfield[todo="valueFilter"]': {
            focus: 'focused',
            change: 'selectedProduct',
            specialkey: 'pressedEnter'
        },
        'transcpurchase grid dataview': {
            refresh: 'refreshView'
        },
        'transcpurchase grid[prop="purchasemaster"]': {
            afterrender: 'addTransactionGrid',
            select: 'selectTransaction'
        },
        'transcpurchase grid[prop="purchasemaster"] toolbar button[todo="add"]': {
            click: 'addTransaction'
        },
        'transcpurchase grid[prop="purchasedetail"] toolbar button[todo="add"]': {
            click: 'showQtyWindow'
        },
        'transcpurchase grid[prop="purchasedetail"] toolbar button[todo="delete"]': {
            click: 'deleteProducts'
        },
        'transcpurchase grid[prop="purchasedetail"] gridcolumn[todo="delete"]': {
            click: 'deleteProduct'
        },
        'transcpurchase grid[prop="purchasedetail"] gridcolumn[todo="misc"]': {
            click: 'showVoidWindow'
        },
        'transcpurchase purchaseUnitWindow toolbar button[action="add"]': {
            click: 'addProduct'
        }
    },
    ...
    addTransactionGrid: async function () {
        ...
        let {Type, SProduct, TransItemDisc, TransItemTax} = grid.up('transcpurchase').stores;
    ...
    selectTransaction: async function (model, record, index) {
        ...
        let {STransactionItem} = grid.up('transcpurchase').stores;
        let store = grid.getStore();
        let count = 1;
        let Obj = {}, charts = [];
    ...
    addTransaction: async function () {
        ...
        let store = grid.getStore();
        let datetime = new Date(A.app.datetime);
        let {Code, Trans} = grid.up('transcpurchase').stores;
    ...
    focused: async function () {
        ...
        let {SProduct} = grid.up('transcpurchase').stores;
        //
    ...
    showQtyWindow: async function (data) {
        ...
        let grid = me.getDetailGrid();
        let purchaseUnitWindow = this.getPurchaseUnitWindow();
        let {SProduct} = grid.up('transcpurchase').stores;
        ...
        purchaseUnitWindow.INPUT = product;
        purchaseUnitWindow.show();
    },
    addProduct: async function (el) {
        let me = this;
        let grid = me.getDetailGrid();
        let store = grid.getStore();
        let {TransItem, TransItemDisc, TransItemTax} = grid.up('transcpurchase').stores;
    ...
    deleteProducts: function (cmp) {
        ...
        let {Product} = grid.up('transcpurchase').stores;
    ...
    init: function () {
        ...
        //
        Ext.create('A.controller.transc.PurchaseUnitWindow').init();
    }
});