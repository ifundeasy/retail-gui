Ext.define('A.controller.master.Taxes', {
    extend: 'Ext.app.Controller',
    views: ['master.Taxes'],
    refs: [
        {ref: 'myGrid', selector: 'masterTaxes grid'},
        {ref: 'mySearchField', selector: 'masterTaxes checkboxlistcombo'},
        {ref: 'mySearchValue', selector: 'masterTaxes textfield[todo=valueFilter]'}
    ],
    ready4Filter: true,
    events: {
        'masterTaxes textfield[todo=valueFilter]': {
            //change: 'filterGrid',
            specialkey: 'pressedEnter'
        },
        'masterTaxes checkboxlistcombo': {
            //change: 'filterGrid',
        ...
        'masterTaxes grid dataview': {
            refresh: 'refreshView'
        },
        'masterTaxes actioncolumn[todo="edit"]': {
            click: 'editRow'
        },
        'masterTaxes actioncolumn[todo="delete"]': {
            click: 'deleteRow'
        },
        'masterTaxes toolbar button[todo="add"]': {
            click: 'addRow'
        },
        'masterTaxes toolbar button[todo="delete"]': {
            click: 'deleteRows'
        },
        'masterTaxes toolbar button[todo="save"]': {
            click: 'saveRows'
        }
    },
    ...
    deselectRow: function (model, record, index) {
        console.log('masterTaxes deselectRow')
    },
    saveRows: function (cmp) {
        let grid = this.getMyGrid();
        let store = grid.getStore();
