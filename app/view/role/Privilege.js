...
Ext.define('A.view.role.Privilege', {
    ...
    alias: 'widget.masterPrivilege',
    initComponent: function () {
        let store = Ext.create('A.store.Rest', {model: 'A.model.Privilege'});
        let actorStore = Ext.create('A.store.Rest', {model: 'A.model.Actor'});
        let moduleStore = Ext.create('A.store.Rest', {model: 'A.model.Module'});
            ...
            items: [
                {
                    ...
                    columns: [
                        ...
                        {text: 'ID', dataIndex: 'id', minWidth: 50, autoSizeColumn: true},
                        {
                            text: 'Actor',
                            dataIndex: 'actor_id',
                            dataSearch: 'actor_name',
                            minWidth: 100,
                            autoSizeColumn: true,
                            renderer: function (val, meta, record, rowIndex) {
                                if (!record.dirty) return record.data.actor_name || '';
                                let idx = actorStore.findExact('id', val);
                                return (idx === -1) ? '' : actorStore.getAt(idx).get('name');
                            },
                            editor: {
                                xtype: 'suggestbox',
                                displayField: 'name',
                        ...
                        },
                        {
                            ...
                            renderer: function (val, meta, record, rowIndex) {
                                ...
                                let idx = moduleStore.findExact('id', val);
                                return (idx === -1) ? '' : moduleStore.getAt(idx).get('name');
                            },
                            editor: {
                                xtype: 'suggestbox',
                                displayField: 'name',
